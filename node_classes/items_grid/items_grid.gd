...
func _ready():
	for item in Stuff.get(list_type):
		...
		item_button.get_theme_stylebox("panel").bg_color = G.RARITY_COLOR[rarity]
		...
		if item.name in DataManager.player_data["available_" + list_type]:
			...
			item_button.get_theme_stylebox("panel").border_color = Color(0.6, 0.6, 0.6)
			move_child(item_button, 0)
		else:
			texture_button.modulate.a = 0.2
			item_button.get_theme_stylebox("panel").border_color = Color(0.3, 0.3, 0.3)
		if item.name == DataManager.player_data.get("selected_" + item_type):
			texture_button.modulate.a = 1
			item_button.get_theme_stylebox("panel").border_color = Color(1, 1, 1)
...
func _on_item_button_pressed(selected_button):
	...
	selected_button.get_theme_stylebox("panel").border_color = Color(1, 1, 1)
