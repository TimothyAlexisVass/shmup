...
@onready var hit_points_bar_foreground = StyleBoxFlat.new()
...
func _ready():
	...
	$HitPoints.position.x = -width/2
	$HitPoints.position.y = -(height/2 + $HitPoints.size.y + 10)
	$HitPoints.size.x = width
	$HitPoints.add_theme_stylebox_override("fill", hit_points_bar_foreground)
...
func take_damage(amount):
	...
	hit_points_bar_foreground.bg_color = Color(red_component, green_component, 0)
