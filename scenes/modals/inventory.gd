...
func _on_inventory_item_pressed(item_details):
	owner.inventory_cannon_name.text = item_details.name + " (" + item_details.shot_type + ")"
	for cannon_property in owner.inventory_cannon_details_grid.get_children():
		...
		if item_details:
			...
			if value is Array or (value && value > 0):
				...
				var mounted_value_node = owner.mounted_cannon_details_grid.get_node(cannon_property.name + "/value")
				var mounted_value = mounted_value_node.text
				var font_color = Color.GREEN
				if mounted_value != "":
					json.parse(mounted_value)
					mounted_value = json.get_data()
					if mounted_value is Array:
						value = value.size() / G.sum(value)
						mounted_value = mounted_value.size() / G.sum(mounted_value)
					if value < mounted_value:
						font_color = Color.RED
					elif value == mounted_value:
						font_color = Color.WHITE
				cannon_property.get_node("value").set("theme_override_colors/font_color", font_color)
